...
public class PaymentServiceFacade {
    ...
    public ApprovalRes approvePipeline(ApprovalReq approvalReq) {
        ...
        log.debug("approval validation [{}] latency = {}", approvalReq.orderId(), LTimer.getDiff(vStartTime));
        if (LTimer.getDiff(vStartTime) >= DB_CONNECTION_TIMEOUT) {
            log.warn("validateApprovalReq() is slow. orderId = {}", approvalReq.orderId());
        }

        long cStartTime = LTimer.getCurrTime();
        Result approveResult = getApproveResult(paymentPayload);
        log.debug("approval call [{}] latency = {}", approvalReq.orderId(), LTimer.getDiff(cStartTime));
        if (LTimer.getDiff(cStartTime) >= PAYMENT_API_TIMEOUT) {
            log.warn("payment API call is slow. orderId = {}", approvalReq.orderId());
        }

        long fStartTime = LTimer.getCurrTime();
        Payment finishedPayment = paymentService.finalizePaymentPayload(approveResult);
        log.debug("approval result apply [{}] latency = {}", approvalReq.orderId(), LTimer.getDiff(fStartTime));
        if (LTimer.getDiff(fStartTime) >= DB_CONNECTION_TIMEOUT) {
            log.warn("finalizePaymentPayload() is slow. orderId = {}", approvalReq.orderId());
        }
