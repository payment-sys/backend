...
public class PaymentServiceFacade {
    ...
    public ApprovalRes approvePipeline(ApprovalReq approvalReq) {
        ...
        PaymentPayload paymentPayload = paymentService.validateApprovalReq(approvalReq);    //1. 승인 요청 검증
        if(LTimer.getDiff(vStartTime) >= DB_CONNECTION_TIMEOUT) {
            log.warn("validateApprovalReq()가 느립니다. orderId = {}", approvalReq.orderId());
        }

        Result approveResult = getApproveResult(paymentPayload);                            //2. 외부 API 호출

        long fStartTime = LTimer.getCurrTime();
        Payment finishedPayment = paymentService.finalizePaymentPayload(approveResult);     //3. 승인 결과 처리
